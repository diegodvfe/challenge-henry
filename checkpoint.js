...
console.log(menormayor([4, 6, 1, 7, 17]))
...
function stringMasLarga(strings) {
  ...
  // Tu código aca:
  var stringsSplit = strings.split(' ');  // Se crea un array de la cadena str separado por espacio (' ')
  var longestWord = 0;
  for (var i = 0; i < stringsSplit.length; i++) {
    if (longestWord < stringsSplit[i].length) {
      longestWord = stringsSplit[i].length;
    }
  }
  return longestWord;
}

console.log(stringMasLarga(['Javascript', 'HTML', 'CSS', 'REACT']));
...
function pluck(array, propiedad) {
  ...
  // Tu código acá:
  const nuevoArray = array.map((a) => a[propiedad]);
  return nuevoArray;
}
...
function crearClasePersona() {
  class Persona {
    ...
    addFriend(nombre, edad) {
      ...
      return `los ${this.nombre} de tus amigos son y su ${this.edad} `
    ...
    getPromedioEdad() {
      ...
      // Tu código aca:var sum = this.edad.reduce((previous, current) => current += previous);
